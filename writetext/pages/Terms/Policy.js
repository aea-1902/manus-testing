...
export default function Policy({onlogout}) {
                ...
                await axios.get(`${AppSettings.API_URL}/Account/GetAccount`,{ httpsAgent: agent  }).then(usr => {
                    if (usr.data.address != null){
                        if (usr.data.address.addressId != "00000000-0000-0000-0000-000000000000")
                            setHasAddress(true)
                            axios.get(`${AppSettings.API_URL}/Policies/GetAcceptedPolicy?type=${PolicyEnum.TermsOfService}`,{ httpsAgent: agent  }).then(r => {
                                if (r.data != null)
                                {
    ...
    // {
    //   axios.post(`${AppSettings.API_URL}/Policies/AcceptPolicy?type=${PolicyEnum.TermsOfService}`,  { httpsAgent }).then(res => {
    ...
    {/* <Header user={loggedUser.firstName} onlogout={onlogout} /> */}
        <Body>
            ...
            <Content backgroundclass={"scrollable-container steps"}>
                    <PolicyComponent type={PolicyEnum.TermsOfService} />
            </Content>
        </Body>
    {/* <Footer/> */}
    </>
