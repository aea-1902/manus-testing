...
import PayPalButtonCredit from '../../components/Paypal/PaypalCredit'
import PayPalButtonSubscription from '../../components/Paypal/PaypalSubscription'
                    ...
                    {(userBilling.allBilling.billingInfo.subscriptions == null && userBilling.credit.membershipType == MemberTypeEnum.SUBSCRIBER) ?
                        ...
                        <>
                            <div className='subscription-tab billing d-flex center mb-10'>
                                ...
                                <>
                                <span className={`${subscriptionType == SubscriptionTypeEnum.PREPAID ? `subscription-active` : ``}`}><a className='m20 mb-0 mt-0' onClick={() => setSubscriptionType(SubscriptionTypeEnum.PREPAID)}>Prepaid</a></span>
                                <span className={`${subscriptionType == SubscriptionTypeEnum.MONTHLY ? `subscription-active` : ``}`}><a className='m20 mb-0 mt-0' onClick={() => setSubscriptionType(SubscriptionTypeEnum.MONTHLY)}>Monthly subscription</a></span>
                                <span className={`${subscriptionType == SubscriptionTypeEnum.ANNUAL ? `subscription-active` : ``}`}><a className='m20 mb-0 mt-0' onClick={() => setSubscriptionType(SubscriptionTypeEnum.ANNUAL)}>Annual subscription</a></span>
                                </>
                                }
                            </div>
                            <div className='subscription-wizard-container p40'>
                                ...
                                <div>
                                    ...
                                    <div key={index}>
                                        <div className={`credit-container mb-20`}>
                                            ...
                                            <div>
                                                ...
                                                <input {...(plan.bestOffer ? {ref: inputRef} : {})} className={`form-check-input ${plan.bestOffer ? `best-offer` : ``}`} type="radio" name="creditCost" id={`creditCost` + index} onChange={e => handleCreditsSub(e)} value={plan.amount} />
                                    ...
                                    {subscriptionType == 0 && planId != "" && userBilling.allBilling.paymentSettings != null &&
                                    <PayPalScriptProvider
                                    options={{
                                    "client-id": userBilling.allBilling.paymentSettings.clientId,
                                    components: "buttons",
                                    currency: userBilling.allBilling.creditPlans.filter(x=>x.id == planId)[0].currency
                                    }}
                                    >
                                    <div id={planId} className='text-center paypal-container'>
                                        <PayPalButtonCredit planId={planId} amount={userBilling.allBilling.creditPlans.filter(x=>x.id == planId)[0].amount} credits={userBilling.allBilling.creditPlans.filter(x=>x.id == planId)[0].credits}/>
                                        {/* <ButtonWrapperCredits planId={planId} amount={userBilling.allBilling.creditPlans.filter(x=>x.id == planId)[0].amount} credits={userBilling.allBilling.creditPlans.filter(x=>x.id == planId)[0].credits} /> */}
                                    </div>
                                    </PayPalScriptProvider>
                                    }
                                    {subscriptionType == 1 && userBilling.allBilling.paymentSettings != null &&
                                        ...
                                        >    
                                    <PayPalButtonSubscription subscription_id={userBilling.allBilling.paymentSettings.monthly.planId} amount={userBilling.allBilling.paymentSettings.monthly.price} subscriptionMessage={setSubscriptionMessage} isMonthly={true}/>                        
                                        </PayPalScriptProvider> 
                                    }
                                    {subscriptionType == 2 && 
                                        <PayPalScriptProvider
                                        ...
                                        >                           
                                        <PayPalButtonSubscription subscription_id={userBilling.allBilling.paymentSettings.annual.planId} amount={userBilling.allBilling.paymentSettings.annual.price} subscriptionMessage={setSubscriptionMessage} isMonthly={false}/>
                                        </PayPalScriptProvider> 
                                    }
                                </div>
                            </div>
