...
export default function MasterTemplate({ userData }) {
  ...
  const [sortableInitAttempts, setSortableInitAttempts] = useState(0);
  const MAX_INIT_ATTEMPTS = 5;
  const INIT_RETRY_DELAY = 500; // 500ms delay between attempts
  ...
  useEffect(() => {
    if (templateObjects.length > 0) {
      saveDraft(templateObjects, 'templateObjectsChange');
    }
  }, [templateObjects]);
  ...
  useEffect(() => {
    if (isDataLoaded && templateObjects.length > 0) {
  ...
  const initializeSortableWithRetry = () => {
    if (sortableInitAttempts >= MAX_INIT_ATTEMPTS) {
      return;
    }

    if (!blocksContainerRef.current || !templateObjects.length || !templateObjects[0]?.blocks) {
      // Increment attempts and retry after delay
      setSortableInitAttempts(prev => prev + 1);
      setTimeout(initializeSortableWithRetry, INIT_RETRY_DELAY);
      return;
    }
  ...
  useEffect(() => {
    const initTemplate = async () => {
      ...
      setLoading(true);
      try {
        // Get the master template using the model endpoint
        const masterTemplate = await getMasterTemplate({ id });
        let updatedTemplateObjects = [masterTemplate];
        updatedTemplateObjects = isKeywordBlockEnabled(updatedTemplateObjects);
        //instantiate warningMessage and errorMessage of blocks
        ...
        setSelectedTemplate(masterTemplate.label);
  ...
  const getMasterTemplates = async () => {
    const response = await ApiService.get('/Templates/master/latest');
  ...
  const handleCheckboxChange = async (index, blockIndex, blockEnabled) => {
    ...
    validateAllHeadings(updatedTemplateObjects);
    if (templateType.toLowerCase().includes('title'))
    {
  ...
  const handleTextStructureChange = (blockIndex, propertyIndex, options) => {
    const updatedTemplateObjects = [...templateObjects];
    const group = updatedTemplateObjects[0].blocks[blockIndex].groups.find(g => g.properties.some(p => p.id === 'text_structure'));
  ...
  const handleTextFieldChange = (blockIndex, propertyIndex, propertyId, value, group) => {
    ...
    for (const group of block.groups) {
      foundProperty = group.properties.find(p => p.id === propertyId);
      if (foundProperty) {
        foundProperty.values = [value.trim()];
        break;
      }
    }
  ...
  const handleAddImageBlock = async (blockIndex) => {
    ...
    setIsImageBlockMax(imageBlocksCount == 10);
  ...
  const handleAddSymbolBlock = async (blockIndex) => {
    ...
    setIsSymbolBlockMax(symbolBlocksCount == 10);
  ...
  const handleAddCustomBlock = async (blockIndex) => {
    ...
    setIsCustomBlockMax(customBlocksCount == 10);
  ...
  const save = async () => {
    ...
    const templateData = {
      ...
      await ApiService.post(`/Templates/master/${templateObjects[0].id}/save`, templateData).then(response => {
    ...
    // TODO: Implement save functionality for master template
  };
        ...
        <div className={`${styles.templatesContainerAdd}`}>
                ...
                {(templateObjects.length > 0 && templateObjects[0].blocks) && templateObjects[0].blocks.map((block, blockIndex) => (
                  <React.Fragment key={`block-${blockIndex}-${block.id}`}>
                    <div 
                      ...
                      // className={`${styles.accordionItemParent} ${blockIndex === 0 ? styles.templatesRowTableBodyRowRowsIconsSortDisabled : ''}`}
                    >
                            ...
                            <div className={styles.propertyGroup}>
                                {block.groups.map((group, groupIndex) => (
                                    <>
                                      ...
                                      {group.properties[0].groupLabel && group.properties[0].groupLabel != '' && <p className={styles.groupLabel}>{group.properties[0].groupLabel}</p>}
          ...
          <div className={styles.templatesContainerRight}>
            <div className={`${styles.templatePreview}`}>
              <PreviewTemplate content={generatedPreview} 
                isPreview={false}
