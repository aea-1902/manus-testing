...
export default function PaypalCredit({planId, amount, credits}) {
  ...
  {!isPending &&
    ...
    {!showError && !ongoingPayment ? 
        <PayPalButtons
            forceReRender={[amount]}
            createOrder={(data,actions) => {
                return axios.post(`${AppSettings.API_URL}/PayPal/order?planId=${planId}`, { httpsAgent: agent })
                .then((order) => {return order.data.order.id})
            }}
            onApprove={async (data) => {

                setOngoingPayment(true)
                // axios.post(`${AppSettings.API_URL}/PayPal/order/${data.orderID}/complete`, { httpsAgent: agent }).then(r => {
